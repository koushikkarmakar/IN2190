...
function TaskTime_recorder()
{
        ...
        (sed -n '/Elapsed time/p' $2 |awk '{print $1 ":" $5 }')>>result_GNU.txt
}		

cat number_GNU.txt | while read line
do
        name_num=$line
        OLD_IFS="$IFS"
        IFS=":"
...
sort -t ":" -k 3n result_GNU.txt -o result_GNU.txt
(sed -n '1p' result_GNU.txt|awk '{print $1 }') > interfile_GNU.txt
sed -i -e "s/:/ /g" interfile_GNU.txt
flags=$(sed -n '1p' interfile_GNU.txt|awk '{print $1}')
rm Makefile
