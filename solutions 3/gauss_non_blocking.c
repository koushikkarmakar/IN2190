...
int main(int argc, char** argv) {
	...
	if(rank == 0) {
		...
		MPI_Irecv(&columns, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &req);
	}	
	...
	if(rank == 0) {
		...
		MPI_Irecv(rhs_local_block, local_block_size, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &req);
	}
	...
	for(process = 0; process < rank; process++) {
		mpi_start = MPI_Wtime();
		MPI_Irecv(pivots, (local_block_size * rows + local_block_size + 1), MPI_DOUBLE, process, process, MPI_COMM_WORLD, &req);
		MPI_Wait(&req, &status);
	...
	for (process = (rank + 1); process<size; ++process) {
		mpi_start = MPI_Wtime();
		MPI_Irecv( accumulation_buffer, (2 * local_block_size), MPI_DOUBLE, process, process, MPI_COMM_WORLD, &req); 
		MPI_Wait(&req, &status);
		mpi_time += MPI_Wtime() - mpi_start;
	...
	if(rank == 0) {
		...
		mpi_start = MPI_Wtime();
		for(i = 1; i < size; i++){
			MPI_Irecv(solution + (i * local_block_size), local_block_size, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, &req);
		}
		mpi_time += MPI_Wtime() - mpi_start;
	} else {
		mpi_start = MPI_Wtime();
		MPI_Isend(solution_local_block, local_block_size, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &req);
		mpi_time += MPI_Wtime() - mpi_start;
	}
