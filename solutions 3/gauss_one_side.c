...
int main(int argc, char** argv) {
	...
	if(rank == 0) {
		...
		matrix_2d_mapped = (double **) malloc(rows * sizeof(double *)); //declare matrix A
		...
		rhs  = (double *)malloc(rows * sizeof(double)); //declare vector b
		for (row = 0; row < rows; row++){
		...
		solution = (double *) malloc (rows * sizeof(double)); //declare vector x 
	}
	...
	MPI_Win win_row, win_column;
	MPI_Win win_matrix_1D_mapped, win_rhs;
	MPI_Win win_pivot, win_accu, win_solution;

	int ranks[size];
	for (i = 0; i < size; i++ ) {ranks[i] = i;}

	//use fence to send other processes the data.
	MPI_Win_create(&rows, 1, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win_row);
	MPI_Win_create(&columns, 1, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &win_column);

	MPI_Win_fence(0, win_row);
	MPI_Win_fence(0, win_column);
	if(rank != 0){
	    MPI_Get(&rows, 1, MPI_INT, 0, 0, 1, MPI_DOUBLE, win_row);
	    MPI_Get(&columns, 1, MPI_INT, 0, 0, 1, MPI_DOUBLE, win_column);
	}
	MPI_Win_fence(0, win_row);
	MPI_Win_fence(0, win_column);
	...
	//use fence to send other processes the data.
	MPI_Win_create(matrix_local_block, local_block_size * rows, 
		sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_matrix_1D_mapped);

	MPI_Win_create(rhs_local_block, local_block_size, sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_rhs);
	
	MPI_Win_fence(0, win_matrix_1D_mapped);
	MPI_Win_fence(0, win_rhs);

	if(rank == 0) {

		for(i = 1; i < size; i++){

			//send space of MLB to other ranks, address starts form the end from matrix_1D_mapped
			MPI_Put((matrix_1D_mapped + (i * (local_block_size * rows))), (local_block_size * rows), MPI_DOUBLE,
			 i, 0, (local_block_size * rows), MPI_DOUBLE, win_matrix_1D_mapped);
		}

		for(i = 1; i < size; i++){
			//send space of RLB to other ranks, address starts form the end rhs
			MPI_Put((rhs + (i * local_block_size)), local_block_size, MPI_DOUBLE,
			 i, 0, local_block_size, MPI_DOUBLE, win_rhs);
		}

		for(i = 0; i < local_block_size * rows; i++){
			//assign element to MLB_rank0
			matrix_local_block[i] = matrix_1D_mapped[i];
		}
		for(i = 0; i < local_block_size; i++){
			//assign element to rhs_local_block_rank0
	...
	MPI_Win_create(pivots, (local_block_size + (rows * local_block_size) + 1), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_pivot );
	MPI_Win_create(accumulation_buffer, (local_block_size * 2), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_accu );
	// printf("Error in 1 %d \n", rank);
	...
	for(process = 0; process < rank; process++) {
		...
		/* Target group consists of the rank who have latest pivots  */
		MPI_Group_incl(comm_group, 1, ranks + process, &group);
		/* Begin the access epoch */
		MPI_Win_start(group, 0, win_pivot);
		/* Get pivot from rank who have latest pivots according to my rank */
	...
	mpi_start = MPI_Wtime();
	/* Origin group consists of the ranks who need pivots */
	MPI_Group_incl(comm_group, size - (rank + 1), ranks + rank + 1 , &group);

	/* Begin the exposure epoch */
	MPI_Win_post(group, 0, win_pivot);
	...
	for (process = (rank + 1); process<size; ++process) {
		...
		/* Target group consists of the rank who have latest accumulation_buffer  */
		MPI_Group_incl(comm_group, 1, ranks + process, &group);

		/* Begin the access epoch */
		MPI_Win_start(group,0,win_accu);

		/* Get pivot from rank who have latest accumulation_buffer according to my rank */
		MPI_Get(accumulation_buffer, (2 * local_block_size), MPI_DOUBLE, process, 0, (2 * local_block_size), MPI_DOUBLE, win_accu);
	
		/* Terminate the access epoch */
		MPI_Win_complete(win_accu);
		mpi_time += MPI_Wtime() - mpi_start;
	...
	mpi_start = MPI_Wtime();
	MPI_Group_incl(comm_group, rank, ranks, &group);
	/* Begin the exposure epoch */
	MPI_Win_post(group, 0, win_accu);
	/* Wait for epoch to end */
	MPI_Win_wait(win_accu);
	mpi_time += MPI_Wtime() - mpi_start;
 
 	MPI_Win_create(solution_local_block, local_block_size, sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &win_solution);
	...
	if(rank == 0) {
		...
		MPI_Win_fence(0, win_solution);

		for(i = 1; i < size; i++){
			MPI_Get(solution + (i * local_block_size), local_block_size, MPI_DOUBLE, i, 0, local_block_size, MPI_DOUBLE, win_solution);
		}
		MPI_Win_fence(0, win_solution);
	...
	MPI_Win_free( &win_row );
  	MPI_Win_free( &win_column);
  	MPI_Win_free( &win_matrix_1D_mapped );
  	MPI_Win_free( &win_rhs);
  	MPI_Win_free(&win_pivot);
	MPI_Win_free(&win_accu);
	MPI_Win_free(&win_solution);
